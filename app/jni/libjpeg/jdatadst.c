...
if (
JFWRITE(dest
->outfile, dest->buffer, OUTPUT_BUF_SIZE) !=
...
if (
JFWRITE(dest
->outfile, dest->buffer, datacount) != datacount)
ERREXIT(cinfo, JERR_FILE_WRITE
);
}
fflush(dest
...
jpeg_stdio_dest(j_compress_ptr
cinfo,
FILE *outfile
) {
my_dest_ptr dest;
...
cinfo->
dest = (struct jpeg_destination_mgr *)
