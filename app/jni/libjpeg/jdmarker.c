...
LOCAL(boolean)
get_sof(j_decompress_ptr
cinfo,
boolean is_prog, boolean
is_arith)
...
TRACEMS4(cinfo,
1, JTRC_SOF, cinfo->unread_marker,
...
cinfo->
comp_info = (jpeg_component_info * )(*cinfo->mem->alloc_small)
...
TRACEMS4(cinfo,
1, JTRC_SOS_PARAMS, cinfo->Ss, cinfo->Se,
cinfo->Ah, cinfo->Al);
...
MEMZERO(&huffval[count], (256 - count) *
SIZEOF(UINT8)
);
...
quant_ptr->quantval[jpeg_natural_order[i]] = (UINT16)
tmp;
}
...
examine_app0(j_decompress_ptr
cinfo,
JOCTET FAR
*data,
unsigned int datalen, INT32
remaining)
...
WARNMS2(cinfo, JWRN_JFIF_MAJOR,
        cinfo
->JFIF_major_version, cinfo->JFIF_minor_version);
...
* (INT32) 3))
TRACEMS1(cinfo,
1, JTRC_JFIF_BADTHUMBNAILSIZE, (int) totallen);
} else if (datalen >= 6 &&
GETJOCTET(data[0])
== 0x4A &&
GETJOCTET(data[1])
...
case 0x10:
TRACEMS1(cinfo,
1, JTRC_THUMB_JPEG, (int) totallen);
break;
case 0x11:
TRACEMS1(cinfo,
1, JTRC_THUMB_PALETTE, (int) totallen);
break;
case 0x13:
TRACEMS1(cinfo,
1, JTRC_THUMB_RGB, (int) totallen);
break;
default:
TRACEMS2(cinfo,
...
examine_app14(j_decompress_ptr
cinfo,
JOCTET FAR
*data,
unsigned int datalen, INT32
...
TRACEMS4(cinfo,
1, JTRC_ADOBE, version, flags0, flags1, transform);
cinfo->
saw_Adobe_marker = TRUE;
cinfo->
Adobe_transform = (UINT8) transform;
} else {
/* Start of APP14 does not match "Adobe", or too short */
TRACEMS1(cinfo,
1, JTRC_APP14, (int) (datalen + remaining));
}
}


METHODDEF(boolean)
get_interesting_appn(j_decompress_ptr
cinfo)
...
case
M_APP0:
        examine_app0(cinfo, (JOCTET
FAR *) b, numtoread, length);
break;
case
M_APP14:
        examine_app14(cinfo, (JOCTET
FAR *) b, numtoread, length);
break;
default:
/* can't get here unless jpeg_save_markers chooses wrong processor */
ERREXIT1(cinfo, JERR_UNKNOWN_MARKER, cinfo
->unread_marker);
break;
}
...
METHODDEF(boolean)
skip_variable(j_decompress_ptr
cinfo)
...
TRACEMS2(cinfo,
1, JTRC_MISC_MARKER, cinfo->unread_marker, (int) length);
...
WARNMS2(cinfo, JWRN_EXTRANEOUS_DATA, cinfo
->marker->discarded_bytes, c);
cinfo->marker->
discarded_bytes = 0;
}
...
METHODDEF(boolean)
read_restart_marker(j_decompress_ptr
cinfo) {
...
TRACEMS1(cinfo,
3, JTRC_RST, cinfo->marker->next_restart_num);
cinfo->
unread_marker = 0;
} else {
...
cinfo->marker->
next_restart_num = (cinfo->marker->next_restart_num + 1) & 7;
...
GLOBAL(boolean)
jpeg_resync_to_restart(j_decompress_ptr
cinfo,
int desired
) {
...
TRACEMS2(cinfo,
4, JTRC_RECOVERY_ACTION, marker, action);
switch (action) {
case 1:
...
get_sos_marker_position(j_decompress_ptr
cinfo,
huffman_index *index
) {
unsigned char *head;
...
reset_marker_reader(j_decompress_ptr
cinfo) {
my_marker_ptr marker = (my_marker_ptr) cinfo->marker;
...
cinfo->
marker = (struct jpeg_marker_reader *) marker;
...
jpeg_set_marker_processor(j_decompress_ptr
cinfo,
int marker_code,
...
marker->process_APPn[marker_code - (int) M_APP0] =
routine;
else
ERREXIT1(cinfo, JERR_UNKNOWN_MARKER, marker_code
);
