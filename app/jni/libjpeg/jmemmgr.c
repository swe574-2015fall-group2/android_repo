...
return (
void FAR
*) (hdr_ptr + 1); /* point to first data byte in pool */
}
...
request_virt_sarray(j_common_ptr
cinfo,
int pool_id, boolean
pre_zero,
JDIMENSION samplesperrow, JDIMENSION
numrows,
JDIMENSION maxaccess
)
...
request_virt_barray(j_common_ptr
cinfo,
int pool_id, boolean
pre_zero,
JDIMENSION blocksperrow, JDIMENSION
numrows,
JDIMENSION maxaccess
)
...
realize_virt_arrays(j_common_ptr
cinfo)
...
sptr->
rows_in_mem = sptr->rows_in_array;
} else {
/* It doesn't fit in memory, create backing store. */
sptr->
rows_in_mem = (JDIMENSION)(max_minheights * sptr->maxaccess);
...
sptr->
mem_buffer = alloc_sarray(cinfo, JPOOL_IMAGE,
                          sptr->samplesperrow, sptr->rows_in_mem);
sptr->
rowsperchunk = mem->last_rowsperchunk;
sptr->
cur_start_row = 0;
sptr->
first_undef_row = 0;
...
/* This buffer fits in memory */
bptr->
rows_in_mem = bptr->rows_in_array;
} else {
/* It doesn't fit in memory, create backing store. */
bptr->
...
bptr->
mem_buffer = alloc_barray(cinfo, JPOOL_IMAGE,
...
do_sarray_io(j_common_ptr
cinfo,
jvirt_sarray_ptr ptr, boolean
writing)
...
do_barray_io(j_common_ptr
cinfo,
jvirt_barray_ptr ptr, boolean
writing)
...
METHODDEF(JSAMPARRAY)
access_virt_sarray(j_common_ptr
cinfo,
...
ptr->
cur_start_row = (JDIMENSION) ltemp;
}
...
jzero_far((
void FAR
*) ptr->mem_buffer[undef_row], bytesperrow);
undef_row++;
}
} else {
if (!writable)
...
sptr->
b_s_open = FALSE;    /* prevent recursive close if error */
...
space_freed = lhdr_ptr->hdr.bytes_used +
              ...
              SIZEOF(large_pool_hdr);
jpeg_free_large(cinfo, (
void FAR
*) lhdr_ptr, space_freed);
mem->total_space_allocated -=
space_freed;
...
space_freed = shdr_ptr->hdr.bytes_used +
              ...
              SIZEOF(small_pool_hdr);
jpeg_free_small(cinfo, (
void *) shdr_ptr, space_freed);
mem->total_space_allocated -=
space_freed;
...
ERREXIT1(cinfo, JERR_OUT_OF_MEMORY,
0);
}
...
mem->
total_space_allocated = SIZEOF(my_memory_mgr);
...
max_to_use *= 1000L;
mem->pub.
max_memory_to_use = max_to_use * 1000L;
}
}
}
#endif
