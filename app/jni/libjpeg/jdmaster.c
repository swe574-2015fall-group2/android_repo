...
cinfo->
output_components = (cinfo->quantize_colors ? 1 :
...
prepare_range_limit_table(j_decompress_ptr
cinfo)
...
master_selection(j_decompress_ptr
cinfo) {
...
cinfo->
enable_external_quant = TRUE;
} else if (cinfo->two_pass_quantize) {
cinfo->
enable_2pass_quant = TRUE;
} else {
cinfo->
enable_1pass_quant = TRUE;
}
...
jinit_d_post_controller(cinfo, cinfo
->enable_2pass_quant);
}
/* Inverse DCT */
...
prepare_for_output_pass(j_decompress_ptr
cinfo) {
my_master_ptr master = (my_master_ptr) cinfo->master;
...
cinfo->progress->
total_passes = master->pass_number +
...
finish_output_pass(j_decompress_ptr
cinfo) {
my_master_ptr master = (my_master_ptr) cinfo->master;
...
cinfo->
master = (struct jpeg_decomp_master *) master;
master->pub.
