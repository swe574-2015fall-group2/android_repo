...
emit_2bytes(j_compress_ptr
cinfo,
int value
)
/* Emit a 2-byte integer; these are always MSB first in JPEG files */
{
emit_byte(cinfo, (value
>> 8) & 0xFF);
emit_byte(cinfo, value
& 0xFF);
}
...
emit_2bytes(cinfo,
3 * cinfo->num_components + 2 + 5 + 1); /* length */
...
num_components;
ci++, compptr++) {
emit_byte(cinfo, compptr
->component_id);
emit_byte(cinfo, (compptr
->h_samp_factor << 4) + compptr->v_samp_factor);
emit_byte(cinfo, compptr
->quant_tbl_no);
}
}
...
emit_adobe_app14(j_compress_ptr
cinfo)
...
write_marker_header(j_compress_ptr
cinfo,
int marker,
...
emit_marker(cinfo, (JPEG_MARKER)
marker);

emit_2bytes(cinfo, (
int) (datalen + 2));    /* total length */
}
...
write_file_header(j_compress_ptr
cinfo) {
...
write_scan_header(j_compress_ptr
cinfo) {
my_marker_ptr marker = (my_marker_ptr) cinfo->marker;
int i;
jpeg_component_info *compptr;
...
write_file_trailer(j_compress_ptr
cinfo) {
...
write_tables_only(j_compress_ptr
cinfo) {
int i;
...
cinfo->
marker = (struct jpeg_marker_writer *) marker;
