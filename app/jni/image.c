...
Java_swe574_boun_edu_androidproject_tools_AndroidUtilities_blurBitmap(JNIEnv
*env,
jclass class, jobject
bitmap,
int radius
) {
if (!bitmap) {
...
if (radius <= 3) {
fastBlur(info
.width, info.height, info.stride, pixels, radius);
} else {
fastBlurMore(info
.width, info.height, info.stride, pixels, radius);
}
...
JNIEXPORT void JNICALL
Java_swe574_boun_edu_androidproject_tools_AndroidUtilities_loadBitmap(JNIEnv
*env,
jclass class, jstring
path,
jobject bitmap,
int scale,
int width,
...
if ((
i = AndroidBitmap_getInfo(env, bitmap, &info)
) >= 0) {
...
if ((
i = AndroidBitmap_lockPixels(env, bitmap, &pixels)
) >= 0) {
...
throwException(env,
"AndroidBitmap_lockPixels() failed ! error=%d", i);
}
...
(*env)->
ReleaseStringUTFChars(env, path, fileName
);
} else {
throwException(env,
"AndroidBitmap_getInfo() failed ! error=%d", i);
}
}
