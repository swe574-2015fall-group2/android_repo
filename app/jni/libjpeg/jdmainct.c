...
alloc_funny_pointers(j_decompress_ptr
cinfo)
...
main->xbuffer[1] = main->xbuffer[0] + cinfo->
num_components;
...
set_wraparound_pointers(j_decompress_ptr
cinfo)
...
i<rgroup;
i++) {
xbuf0[i - rgroup] = xbuf0[
rgroup *(M
+ 1) + i];
xbuf1[i - rgroup] = xbuf1[
rgroup *(M
+ 1) + i];
xbuf0[
rgroup *(M
...
set_bottom_pointers(j_decompress_ptr
cinfo)
...
start_pass_main(j_decompress_ptr
cinfo,
...
main->pub.
process_data = process_data_context_main;
...
process_data_simple_main(j_decompress_ptr
cinfo,
...
process_data_context_main(j_decompress_ptr
cinfo,
JSAMPARRAY output_buf, JDIMENSION
*out_row_ctr,
JDIMENSION out_rows_avail
