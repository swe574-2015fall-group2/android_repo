...
start_pass_huff_decoder(j_decompress_ptr
cinfo) {
...
jpeg_make_d_derived_tbl(j_decompress_ptr
cinfo,
boolean isDC,
int tblno,
        d_derived_tbl
...
jpeg_huff_decode(bitread_working_state
*state,
register bit_buf_type get_buffer,
register int bits_left,
        d_derived_tbl
...
get_huffman_decoder_configuration(j_decompress_ptr
cinfo,
...
jpeg_get_huffman_decoder_configuration(j_decompress_ptr
cinfo,
huffman_offset_data *offset
) {
huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
...
if (entropy->restarts_to_go == 0) if (!
...
offset->
restarts_to_go = (unsigned short) entropy->restarts_to_go;
offset->
next_restart_num = cinfo->marker->next_restart_num;
...
configure_huffman_decoder(j_decompress_ptr
cinfo,
huffman_offset_data offset
) {
huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
...
jpeg_configure_huffman_decoder(j_decompress_ptr
cinfo,
huffman_offset_data offset
) {
huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
...
unread_marker = 0;
entropy->
restarts_to_go = offset.restarts_to_go;
cinfo->marker->
next_restart_num = offset.next_restart_num;
...
BITREAD_LOAD_STATE(cinfo, entropy
->bitstate);
 ...
 */
(*block)[jpeg_natural_order[k]] = (JCOEF)
s;
} else {
if (r != 15)
break;
...
BITREAD_SAVE_STATE(cinfo, entropy
->bitstate);
...
METHODDEF(boolean)
decode_mcu_discard_coef(j_decompress_ptr
cinfo) {
huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
int blkn;
BITREAD_STATE_VARS;
...
cinfo->
entropy = (struct jpeg_entropy_decoder *) entropy;
entropy->pub.
start_pass = start_pass_huff_decoder;
entropy->pub.
decode_mcu = decode_mcu;
entropy->pub.
decode_mcu_discard_coef = decode_mcu_discard_coef;
entropy->pub.
configure_huffman_decoder = configure_huffman_decoder;
entropy->pub.
...
jpeg_create_huffman_index(j_decompress_ptr
cinfo,
huffman_index *index
) {
int i, s;
...
index->
scan = (huffman_scan_header *) malloc(index->scan_count
                                      * sizeof(huffman_scan_header));
index->scan[0].
offset = (huffman_offset_data **) malloc(cinfo->total_iMCU_rows
                                         * sizeof(huffman_offset_data * ));
index->scan[0].prev_MCU_offset.
bitstream_offset = 0;
index->
MCU_sample_size = DEFAULT_MCU_SAMPLE_SIZE;

index->
mem_used = sizeof(huffman_scan_header)
           + cinfo->total_iMCU_rows * sizeof(huffman_offset_data * );
}

GLOBAL(void)

jpeg_destroy_huffman_index(huffman_index
*index) {
int i, j;
for (
i = 0;
...
jset_input_stream_position(j_decompress_ptr
cinfo,
int offset
) {
if (cinfo->src->seek_input_data) {
cinfo->src->
seek_input_data(cinfo, offset
);
} else {
cinfo->src->
bytes_in_buffer = cinfo->src->current_offset - offset;
cinfo->src->
next_input_byte = cinfo->src->start_input_byte + offset;
}
}
...
jset_input_stream_position_bit(j_decompress_ptr
cinfo,
int byte_offset,
int bit_left, INT32
buf) {
...
jget_input_stream_position(j_decompress_ptr
cinfo) {
return cinfo->src->current_offset - cinfo->src->
bytes_in_buffer;
}
